...
import { Warehouse, Download, AlertTriangle, Image } from 'lucide-react';
...
const StockPage: React.FC = () => {
    ...
    const handleExportJPG = async () => {
        if (!exportAreaRef.current) {
            console.error("Elemento de contenido principal no encontrado para exportar.");
            alert("No se pudo encontrar el área para exportar.");
            return;
        }
        ...
        try {
            const canvas = await html2canvas(exportAreaRef.current, {
                backgroundColor: '#1a202c', // Matches app's primary background
                useCORS: true,
                scale: 2 // Higher resolution
            });
            const link = document.createElement('a');
            const safeFilename = selectedIngredientName.replace(/ /g, '_') || 'general';
            link.download = `reporte_stock_${safeFilename}_${new Date().toISOString().split('T')[0]}.jpg`;
            link.href = canvas.toDataURL('image/jpeg', 0.9); // Quality setting for JPG
            link.click();
        } catch (error) {
        ...
        <div className="space-y-8">
            <Card className="mb-8">
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                 ...
                 <div className="mt-4 pt-4 border-t border-accent flex flex-wrap gap-2 justify-between">
                    <Button 
                    ...
                    >
                     ...
                     <div className="flex flex-wrap gap-2">
                        ...
                        <Button onClick={handleExportJPG} disabled={isExportingJPG} className="w-full sm:w-auto">
                            <Image className="mr-2" />
                            {isExportingJPG ? 'Exportando...' : 'Exportar a JPG'}
                        </Button>
                    </div>
                </div>
            </Card>
